// Arabic letter-joining (presentation forms) for PDF rendering
// Adapted from arabic-persian-reshaper (MIT), Alex Clay / Shen Yiming
...
return {convertArabic:convertArabic};
})();
function reshapeAr(txt){
  if(txt==null)return '';
  txt=String(txt);
  // only process if it contains Arabic
  if(!/[\u0600-\u06FF]/.test(txt))return txt;
  try{
    var shaped=ArabicReshaper.convertArabic(txt);
    // reverse the string for RTL display in jsPDF (which lays out LTR)
    // keep latin/number runs in order
    var tokens=shaped.split(/(\s+)/);
    tokens.reverse();
    return tokens.join('');
  }catch(e){return txt;}
}