...
document.addEventListener('keydown',function(e){if(G('np-ov')&&!G('np-ov').classList.contains('hidden'))return;var a=document.activeElement,tag=a?a.tagName:'';if(tag==='INPUT'||tag==='SELECT'||tag==='TEXTAREA')return;if(e.key==='Enter'||e.key==='Tab'){e.preventDefault();markUSBconnected();doScan();return;}if(e.key==='Backspace'){e.preventDefault();var s=G('scan-inp');if(s)s.value=s.value.slice(0,-1);return;}if(e.key&&e.key.length===1){e.preventDefault();var s2=G('scan-inp');if(s2){s2.value+=e.key;clearTimeout(_scanInputTimer);_scanInputTimer=setTimeout(function(){var v=(s2.value||'').trim();if(v.length>=4){markUSBconnected();handleScan(v);}},180);}}},true);
window.addEventListener('focus',function(){setTimeout(focusScan,60);});
function markUSBconnected(){var dot=G('usb-mini-dot'),badge=G('usb-mini-badge');if(dot)dot.style.background='#1a7a4a';if(badge){badge.textContent='Connecté ✅';badge.className='badge b-ok';}}
...
function setupScanInput(){var s=G('scan-inp');if(!s)return;s.addEventListener('input',function(){clearTimeout(_scanInputTimer);if(/[\r\n\t]/.test(s.value)){var vv=s.value.replace(/[\r\n\t]/g,'').trim();s.value=vv;if(vv.length>=4){markUSBconnected();handleScan(vv);return;}}var val=(s.value||'').trim();if(val.length>=4){_scanInputTimer=setTimeout(function(){var v=(s.value||'').trim();if(v.length>=4){markUSBconnected();handleScan(v);}},180);}});}
